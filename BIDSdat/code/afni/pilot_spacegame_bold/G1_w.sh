...
#set test directory
set test_dir = $bidsdir/derivatives/analyses/pilot_spacegame_bold/level2/1sampleT

# set R dir
set r_dir = $bidsdir/derivatives/analyses/pilot_spacegame_bold/R
...
set maps = ( High-Low_allPS_GLT Food-Office_GLT )
...
foreach map ( $maps )
	...
	gen_group_command.py -command 3dttest++                                             \
		-write_script $map_folder/w_${output_name}         \
		-prefix  w_${output_name}                    \
		...
		-options                                                       \
			...
			-covariates $map_folder/fmri_covariates.csv"'[0,1,2,4,5,23]'"

	gen_group_command.py -command 3dttest++                                             \
		-write_script $map_folder/w-cut_${output_name}         \
		-prefix  w-cut_${output_name}                   \
		-dsets ${lev1_dir}/*/${lev1_results}/stats.sub-???+tlrc.HEAD       \
    	-dset_sid_list $index 					\
	 	-set_labels $map                                                 \
     	-subs_betas "${map}"'#'0_Coef                                    \
		-options                                                       \
            -mask $map_folder/WG_mask0.8+tlrc.HEAD	\
			-covariates $map_folder/fmri_covariates.csv"'[0,1,2,4,5,30]'"
