...
roi_dir="$f31_dir/rois/" #directory with ROI spheres
submask_dir="$f31_dir/foodcue_masks/$subID" #directory to store subject's food cue and coverage masks
...
if [ ! -d "$submask_dir" ]; then
        echo "making mask directory"
		mkdir -p "$submask_dir"
fi

###################### Copy masks from fmriprep  ########################### 
cp $subfunc_dir/${subID}_ses-1_task-foodcue_run-?_space-MNIPediatricAsym_cohort-3_res-1_desc-brain_mask.nii.gz $submask_dir
...
run_array=() #initialize array

# loop through files and extract run number
...
for run in "${run_array[@]}"; do
    ...
    if [ ! -f "$submask_dir/${subID}_run${run}_roi_coverage_mask+tlrc.BRIK" ]; then
        ...
        3dMean \
            ...
            $roi_dir/rois_mask_neg2+tlrc $submask_dir/${subID}_ses-1_task-foodcue_run-${run}_space-MNIPediatricAsym_cohort-3_res-1_desc-brain_mask.nii.gz
    else
...
#    echo "checking ROI coverage for $subID run $run"
    ...
    if [ ! -f "$f31_dir/foodcue_masks/summary_coverage_status.txt" ]; then

        # create summary file: write columns names, -e  enables the interpretation of backslash escapes for \t as a tab separator.
        echo -e "sub\trun\tcoverage_status" > $f31_dir/foodcue_masks/summary_coverage_status.txt

        # add subID, run, and coverage status to summary file
        echo -e "$subID\t$run\t$coverage_status" >> $f31_dir/foodcue_masks/summary_coverage_status.txt
    
    # if summary file does exist
    else

        # check if subID and run already in the summary file
        if grep -q -P "^$subID\t$run" "$f31_dir/foodcue_masks/summary_coverage_status.txt"; then
	 
	    echo "deleting coverage status for $subID run $run"

            # delete the row containing subID and run
            sed -i "/^$subID\t$run/d" "$f31_dir/foodcue_masks/summary_coverage_status.txt"
