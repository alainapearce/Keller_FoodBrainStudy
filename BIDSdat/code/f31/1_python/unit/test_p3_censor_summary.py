...
from p3_censor_summary import _get_food_onsetTRs
from p3_censor_summary import _gen_food_TR_list
from p3_censor_summary import _gen_sub_cen_sum
...
def confound_dat_fixture():
    ...
    confound_dat.columns = ['framewise_displacement', 'non_steady_state_outlier00']
    # use np.r_ to concatenate array slices. without np.r_, slices will be inclusive
    confound_dat.at[np.r_[:20, 40:46],["rmsd"]] = .5 # set rows 0:20 & 40:46 to have rmsd of .5
    confound_dat.at[np.r_[75:79],["non_steady_state_outlier00"]] = 1    # set rows 75:79 to have framewise displacement of 1.1

    return confound_dat

@pytest.fixture
def food_TR_list_fixture():

    # Make list of 1s and 0s, where 1 indicates TR in a food block, 0 indicates TR of no interest
    food_TR_list = [0] * 80 # make list of zeros
    food_TR_list[2:11] = [1, 1, 1, 1, 1, 1, 1, 1, 1]    #At indices 2:11 in r_int_list, set value to 1
    food_TR_list[15:24] = [1, 1, 1, 1, 1, 1, 1, 1, 1]   #At indices 15:24 in r_int_list, set value to 1
    food_TR_list[28:37] = [1, 1, 1, 1, 1, 1, 1, 1, 1]   #At indices 28:37 in r_int_list, set value to 1
    food_TR_list[41:50] = [1, 1, 1, 1, 1, 1, 1, 1, 1]   #At indices 41:50 in r_int_list, set value to 1
