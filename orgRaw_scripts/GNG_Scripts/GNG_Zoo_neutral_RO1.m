...
function GNG_Zoo_neutral_RO1(parID, session)
...
%(individual path info '/Box Sync/b-childfoodlab Shared/Active_Studies/RO1_Brain_Mechanisms_IRB_5357/Participant Data/GNG/Scripts')
...
for p = 1:npar
    ...
    if sum(par_ind & session_ind) > 0
        ...
        %session
        today = date;
        writetable(GNG_Zoo_ProcData, [sprintf('GNG_Zoo_Processed_%s_%s_%s.txt', parID_str, session_str, today)], 'WriteRowNames',true)
        %%
        ...
        summary_data.Session(row_count) = session;
        ...
        for b = 0:length(unique(GNG_Zoo_TaskData.Block))
            ...
            summary_data(row_count, dat_start:dat_end) = dat_tab;
            ...
            if b == 0
                ...
                DDM_summary_data.ParID(row_count) = parID_num;
                DDM_summary_data.nGo_Hit(row_count) = nGo_Hit;
                DDM_summary_data.nNoGo_FA(row_count) = nNoGo_FA;

                %get quantiles for Go Hit RT
                DDM_summary_data(row_count, 3:7) = num2cell(quantile(block_data_Go.RT, [.1, .3, .5, .7, .9]));
                ...
                if nNoGo_FA > 0
                    ...
                    DDM_summary_data.RTq50NoGo(row_count) = quantile(block_data_NoGo_FA.RT, .5);
                else
                    DDM_summary_data.RTq50NoGo(row_count) = NaN;
                end
            end    
        end   
        ...
        summary_data.z_Hit(row_count) = norminv(summary_data.pGo_Hit(row_count));
        summary_data.z_FA(row_count) = norminv(summary_data.pNoGo_FA(row_count));
        ...
        %for z in that palce
        if summary_data.pGo_Hit(row_count) == 1
            pHit_mm = (summary_data.nGo(row_count)-0.5)/summary_data.nGo(row_count);
            summary_data.z_Hit_mm(row_count) = norminv(pHit_mm);
        else
            pHit_mm = summary_data.pGo_Hit(row_count);
            summary_data.z_Hit_mm(row_count) = norminv(pHit_mm);
        end

        if summary_data.pNoGo_FA(row_count) == 0
            pFA_mm = 0.5/summary_data.nNoGo(row_count);
            summary_data.z_FA_mm(row_count) = norminv(pFA_mm);
        else
            pFA_mm = summary_data.pNoGo_FA(row_count);
            summary_data.z_FA_mm(row_count) = norminv(pFA_mm);
        end
        ...
        %FA rates
        nHit_ll = summary_data.nGo_Hit(row_count)+0.5;
        nGo_ll = summary_data.nGo(row_count) + 1;
        nFA_ll = summary_data.nNoGo_FA(row_count)+0.5;
        nNoGo_ll = summary_data.nNoGo(row_count) + 1;
        pHit_ll = nHit_ll/nGo_ll;
        pFA_ll = nFA_ll/nNoGo_ll;
        summary_data.z_Hit_ll(row_count) = norminv(pHit_ll);
        summary_data.z_FA_ll(row_count) = norminv(pFA_ll);

        %calculate sensory sensitivity d'
        summary_data.d_prime_mm(row_count) = summary_data.z_Hit_mm(row_count) - summary_data.z_FA_mm(row_count);
        summary_data.d_prime_ll(row_count) = summary_data.z_Hit_ll(row_count) - summary_data.z_FA_ll(row_count);

        %calculate nonparametric sensory sensitivity A':
        %0.5+[sign(H-FA)*((H-FA)^2 + |H-FA|)/(4*max(H, FA) - 4*H*FA))
        summary_data.A_prime_mm(row_count) = .5 + (sign(pHit_mm-pFA_mm)*(((pHit_mm-pFA_mm)^2+abs(pHit_mm - pFA_mm))/(4*max(pHit_mm, pFA_mm) - 4*pHit_mm*pFA_mm)));
        summary_data.A_prime_ll(row_count) = .5 + (sign(pHit_ll-pFA_ll)*(((pHit_ll-pFA_ll)^2+abs(pHit_ll - pFA_ll))/(4*max(pHit_ll, pFA_ll) - 4*pHit_ll*pFA_ll)));

        %calculate c (criterion
        summary_data.c_mm(row_count) = (norminv(pHit_mm) + norminv(pFA_mm))/2;
        summary_data.c_ll(row_count) = (norminv(pHit_ll) + norminv(pFA_ll))/2;

        %calculate Grier's Beta--beta", a nonparametric response bias
        summary_data.Grier_beta_mm(row_count) = sign(pHit_mm-pFA_mm)*((pHit_mm*(1-pHit_mm)-pFA_mm*(1-pFA_mm))/(pHit_mm*(1-pHit_mm)+pFA_mm*(1-pFA_mm)));
        summary_data.Grier_beta_ll(row_count) = sign(pHit_ll-pFA_ll)*((pHit_ll*(1-pHit_ll)-pFA_ll*(1-pFA_ll))/(pHit_ll*(1-pHit_ll)+pFA_ll*(1-pFA_ll)));
